...
This RFC defines the **multi-modal data processing model** for HyperFrame.

Leveraging **Lance’s AI-native, columnar design**, HyperFrame supports images, video, audio, text, and other modalities as **queryable, indexable, and semantically interpretable data**, not opaque payloads.
...
HyperFrame supports the following **modal entities**:
...
Multi-modal data MUST be stored in **columnar form**:

* Media payloads stored as Lance blobs or chunked arrays
* Metadata stored in aligned columns
* Temporal slicing supported natively

Rules:

* No row-local blob packing
* Zero-copy reads preferred
* Partial reads MUST be supported
...
This RFC defines **multi-modal cognition** in HyperFrame.

> **Tables store facts.
> Graphs store relationships.
> Multi-modal data stores perception.
> RFC-0014 makes perception computable.**
