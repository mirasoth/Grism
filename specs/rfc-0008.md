...
Operators exchange **Arrow RecordBatch**:
...
* Arrow columnar layout is mandatory
* Row ordering is undefined unless specified
* Batches may be empty (termination signal)
* Zero-copy sharing preferred when possible

---

### 6.2 PhysicalSchema

Based on Arrow Schema:
...
| Binary   | No       | Union                         |
| Expand   | No       | AdjacencyExpand, RoleExpand    |
| Blocking | Yes      | Aggregate, Sort               |
| Sink     | Yes      | Collect, Materialize          |

Blocking operators MUST explicitly declare blocking behavior.

**Important**: There is no Join operator. Relational composition is handled via Expand variants.
...
Traverses hyperedges via role bindings as defined in architecture (Section 3.3).
...
* Binary vs n-ary execution is executor-defined

Expand is the **semantic anchor** of graph execution and the sole traversal primitive.
