# RFC-0001: HyperFrame Logical Model and Execution Architecture
...
This RFC specifies the **HyperFrame** data model and its execution architecture. HyperFrame is a hypergraph-first logical abstraction designed to unify expressive n-ary relational semantics with low-latency interactive graph queries and scalable graph computation. The document defines the core invariants, execution projections, physical layouts, and planning rules that enable Cypher compatibility without sacrificing hypergraph correctness.
...
HyperFrame is introduced to resolve this tension.
...
## 4. Core Abstraction: HyperFrame

### 4.1 Definition

**HyperFrame** is the canonical logical data model.

> HyperFrame is a hypergraph-backed, relationally-executable, AI-native graph container.

It plays a role analogous to `DataFrame` in tabular systems.

### 4.2 Conceptual Mapping

| Tabular Systems | HyperFrame            |
| --------------- | --------------------- |
| Row             | Hyperedge             |
...
1. HyperFrame is the **only source of truth**
...
Cypher assumes a binary property graph. HyperFrame generalizes this via *projection*:
...
HyperFrame may expose a native hypergraph query language (e.g., AnkQL) that operates directly on n-ary relations without projection.

---

## 6. The Join Problem

Executing HyperFrame queries purely via relational joins is:
...
HyperFrame uses a **single logical model with multiple physical execution backends**.
...
> Adjacency is an index. HyperFrame is the model.
...
HyperFrame supports explicit or inferred execution modes:
