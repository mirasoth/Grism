# RFC-0002: HyperFrame Logical Algebra & Formal Semantics
...
This RFC defines the **logical algebra and formal semantics** of HyperFrame.
...
This document establishes HyperFrame’s **semantic contract**: all language frontends, optimizers, physical execution engines, and storage backends MUST preserve the semantics defined herein.
...
* The **logical data model** exposed by HyperFrame
...
| Term             | Meaning                                                          |
| ---------------- | ---------------------------------------------------------------- |
| **HyperFrame**   | The canonical logical container representing a set of hyperedges |
...
A HyperFrame exposes a **columnar view**:
...
Column names MUST be globally unique within a HyperFrame scope.

---

## 5. Logical Algebra Overview

HyperFrame defines a **closed logical algebra**.

> Every operator consumes one or more HyperFrames and produces a HyperFrame.

No operator mutates input HyperFrames.
...
```
Scan(source, predicate?) → HyperFrame
```

**Semantics**

* Produces a HyperFrame containing all hyperedges from `source`
...
```
Expand(input, pattern, direction?) → HyperFrame
```
...
```
Filter(input, predicate) → HyperFrame
```
...
```
Project(input, columns) → HyperFrame
```
...
```
Rename(input, mapping) → HyperFrame
```
...
```
Join(left, right, condition) → HyperFrame
```
...
```
Aggregate(input, keys, functions) → HyperFrame
```
...
```
Infer(input, ruleset) → HyperFrame
```
...
* HyperFrame as the **source of truth**
* Adjacency as a **projection**

RFC-0002 defines:

* The **meaning** of HyperFrame operations
...
This RFC establishes HyperFrame’s **semantic bedrock**.
...
> **HyperFrame is not a graph that happens to support joins.
