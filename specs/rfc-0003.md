...
This RFC defines the **expression system and type model** for HyperFrame.
Expressions are the smallest executable semantic units used in predicates, projections, joins, aggregations, and inference rules.
...
Refer to columns in the current HyperFrame scope.
...
* Name resolution is lexical (RFC-0002)
...
| `ID`        | Entity or hyperedge identifier |
...
| Type           | Description                 |
| -------------- | --------------------------- |
| `Embedding[d]` | Fixed-dimension vector      |
| `Tensor`       | Higher-order numeric tensor |
| `Score`        | Comparable scalar score     |

Distance and similarity functions are defined as pure functions.
...
* Expressions are evaluated against a HyperFrame row
...
* **RFC-0001**: Defines HyperFrame as the container
...
This RFC defines the **semantic atoms** of HyperFrame.
Expressions are where *logic becomes executable*.

> **If RFC-0002 defines “what a query means”,
> RFC-0003 defines “how meaning is computed—safely, deterministically, and portably”.**
