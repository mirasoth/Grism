...
impl LogicalExpr {
    ...
    pub fn function(func: FuncExpr) -> Self {
        Self::Function(func)
    }

    /// Create an aggregate expression.
    pub fn aggregate(agg: AggExpr) -> Self {
        Self::Aggregate(agg)
    }
    ...
    pub fn try_as_literal(&self) -> Option<&Value> {
        match self {
    ...
    pub fn resolve_type(&self, schema: &Schema) -> Option<DataType> {
        match self {
            ...
            Self::Wildcard | Self::QualifiedWildcard(_) => None,
            Self::Subquery(_) | Self::Exists { .. } => Some(DataType::Bool),
            Self::Placeholder { data_type, .. } => data_type.clone(),
            Self::SortKey { expr, .. } => expr.resolve_type(schema),
        }
    }
}
    ...
    match value {
        ...
        Value::Array(arr) => {
            if let Some(first) = arr.first() {
                DataType::Array(Box::new(value_to_data_type(first)))
            } else {
...
mod tests {
    ...
    fn test_column_refs_collection() {
        let expr = LogicalExpr::column("a")
            .add(LogicalExpr::column("b"))
            .mul(LogicalExpr::column("c"));
    ...
    fn test_contains_aggregate() {
        let simple = LogicalExpr::column("x").add(LogicalExpr::literal(1i64));
    ...
    #[test]
    fn test_determinism() {
        let deterministic = LogicalExpr::column("x").add(LogicalExpr::literal(1i64));
